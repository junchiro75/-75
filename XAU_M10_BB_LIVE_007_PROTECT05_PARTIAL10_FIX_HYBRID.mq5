//+------------------------------------------------------------------+
//| XAU_M10_BB_LIVE_007_PROTECT05_PARTIAL10_FIX_HYBRID.mq5           |
//| HYBRID variant of 007, built on top of the BUYONLY logic.        |
//|                                                                    |
//| - BEAR signal bar (down breakout): UNCHANGED from BUYONLY --      |
//|   wait for +0.95R extension then -0.10R pullback, then enter      |
//|   countertrend BUY. This path is real-MT5 validated.               |
//| - BULL signal bar (up breakout): instead of queuing a countertrend|
//|   SELL setup (which BUYONLY disables via AllowShort=false and      |
//|   simply drops), this variant enters a TREND-FOLLOWING BUY         |
//|   immediately on the first tick after the bull bar closes -- no    |
//|   extension/pullback wait. Final TP is still anchored to the       |
//|   signal bar's own close +0.90R (same convention as the            |
//|   countertrend path), not to the entry price.                      |
//|                                                                    |
//| Both paths always resolve to BUY, so this EA is structurally       |
//| BUY-only -- there is no AllowShort input because there is no code  |
//| path left that could ever open a SELL.                             |
//|                                                                    |
//| CAUTION: the immediate-BUY-on-bull-breakout idea has only been     |
//| tested in a Python tick-replay simulation so far (not yet run      |
//| through MT5 Strategy Tester). That simulation showed a meaningful  |
//| improvement over BUYONLY alone, but this project has already seen  |
//| one case where Python-simulated results diverged materially from  |
//| real MT5 execution -- treat this build as UNVALIDATED until you    |
//| have run it in the Strategy Tester (Every tick based on real       |
//| ticks, EnableLiveOrders=true) and compared the report.             |
//|                                                                    |
...
input long MagicNumber=95011207; // distinct from original 007 (95011007) and BUYONLY 007
                                  // (95011107) so all three can run side by side
...
struct Setup{
 datetime sig,ct,exp;
 int sd;                 // always -1 (bear) now -- bull signals are handled immediately, never queued
...
// Shared order opener. dir is always +1 in this build (both the immediate
// bull-breakout entry and the bear-signal countertrend entry resolve to BUY).
bool PlaceEntry(int dir,double R,double sigclose,MqlTick &tk){
 ulong old; if(OwnPosition(old))return false; // strict own-Magic MAX1
 double entry=(dir==1?tk.ask:tk.bid);
 double sl=entry-dir*InitialSL_R*R;
 double finaltp=sigclose+dir*SignalOppositeTP_R*R;
 trade.SetExpertMagicNumber(MagicNumber);
 trade.SetTypeFillingBySymbol(_Symbol);
 bool ok=false;
 if(EnableLiveOrders){
  ok=(dir==1)?trade.Buy(Lots,_Symbol,0,sl,finaltp,"LIVE007_HYBRID")
             :trade.Sell(Lots,_Symbol,0,sl,finaltp,"LIVE007_HYBRID");
 }else{
 ...
 if(!ok){Print("LIVE007 | ENTRY FAILED | retcode=",trade.ResultRetcode()," ",trade.ResultRetcodeDescription());return false;}
 ulong t;
 ...
 tracked_R=R;tracked_sigclose=sigclose;tracked_dir=dir;protection_armed=false;managed_partial=false;
 SaveTrack();
...
void NewBar(MqlTick &tk){
 ...
 if(bull){
  // Bull breakout: trend-follow immediately instead of queuing a countertrend SELL.
  Print("LIVE007 | SIGNAL | BULL R=",DoubleToString(R,2)," -> immediate trend-follow BUY");
  PlaceEntry(+1,R,c,tk);
  return;
 }

 // Bear signal: unchanged extension+pullback wait -> countertrend BUY.
 int n=ArraySize(S);ArrayResize(S,n+1);
 S[n].sig=st;S[n].ct=st+PeriodSeconds(PERIOD_M10);S[n].exp=S[n].ct+MaxExtensionHours*3600;
 S[n].sd=-1;S[n].R=R;S[n].o=o;S[n].h=h;S[n].l=l;S[n].c=c;S[n].extreme=c;S[n].ext=false;
 Print("LIVE007 | SIGNAL | BEAR R=",DoubleToString(R,2));
}

bool SendEntry(int i,MqlTick &tk){
 int dir=-S[i].sd; // bear signal -> BUY (bull signals never reach this pipeline in this build)
 bool result=PlaceEntry(dir,S[i].R,S[i].c,tk);
 DS(i); // consume the setup whether live, dry, or MAX1-blocked
 return result;
}
...
int OnInit(){
 ...
 trade.SetExpertMagicNumber(MagicNumber);
 Print("LIVE007 HYBRID | START | M10 | bear->ext+pullback->BUY, bull->immediate trend BUY | Lots=",DoubleToString(Lots,2),
 ...
 Print("LIVE007 | NOTE | UNVALIDATED build: bull->immediate-BUY idea only tested in Python simulation so far");
