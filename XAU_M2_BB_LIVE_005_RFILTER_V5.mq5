//+------------------------------------------------------------------+
//| XAU_M2_BB_LIVE_005_RFILTER_V5.mq5                                |
...
bool OpenCountertrend(Setup &s,MqlTick &tick)
{
   ...
   if(!EnableLiveOrders)
   {
      ...
      v_sl=UseFixedSL ? (v_entry-dir*FixedSL_Points*_Point) : (v_entry-dir*InitialSL_R*v_R);
   ...
   double sl=UseFixedSL ? (ref-dir*FixedSL_Points*_Point) : (ref-dir*InitialSL_R*s.R);
...
int OnInit()
{
   ...
   Log("NOTE","RFILTER5 build: RFILTER4 + TightenSLOnTP1/TightenedSL_R (tighten SL on TP1 touch instead of moving to Lock_R)");
