#!/usr/bin/env python3
"""Run the "bear signal -> next bar bullish -> immediate BUY, else
normal extension/pullback wait" idea over all monthly tick files, for
both 005-style (M2) and 007-style (M10) exit economics."""
...
from engine.fastconfirm_runner import FastConfirmRunner
...
def main():
    ...
    ap.add_argument('--results-dir', default=os.path.join(os.path.dirname(__file__), 'results_fastconfirm'))
    ...
    rows = []
    for name in ['005_fastconfirm', '007_fastconfirm']:
    ...
    report.to_csv(os.path.join(args.results_dir, 'summary_fastconfirm.csv'), index=False)
