#!/usr/bin/env python3
"""Run the hybrid bear->wait->BUY / bull->immediate-BUY idea over all
monthly tick files, for both 005-style (M2) and 007-style (M10)."""
...
from engine.hybrid_runner import HybridRunner
...
def main():
    ...
    ap.add_argument('--results-dir', default=os.path.join(os.path.dirname(__file__), 'results_hybrid'))
    ...
    rows = []
    for name in ['005_hybrid', '007_hybrid']:
    ...
    report.to_csv(os.path.join(args.results_dir, 'summary_hybrid.csv'), index=False)
