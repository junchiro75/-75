//+------------------------------------------------------------------+
//| XAU_M2_BB_LIVE_005_TREND_V1.mq5                                  |
//| Trend-following variant of 005_RFILTER_V1. Adds TrendFollow: the  |
//| SAME setup detection (impulsive M2 candle breaks both BB20+BB4 -> |
//| extension by ExtensionR*R -> pullback by PullbackR*R) but enters  |
//| WITH the original impulsive direction instead of fading it, i.e.  |
//| "buy the dip in an uptrend" continuation instead of countertrend. |
//| Rationale: on NAS100/BTC, trend-following (MA120 V12) clearly     |
//| outperformed countertrend (005-family) -- this tests whether      |
//| flipping 005's OWN entry direction, keeping its setup-quality     |
//| logic and MinR_Points filter, captures that same trend edge on a  |
//| much faster (M2) timeframe. TrendFollow=false reproduces          |
//| 005_RFILTER_V1 exactly (countertrend/fade, as validated on gold). |
//| ---- inherited from 005_RFILTER_V1.mq5 / 005_FINAL_BUYONLY.mq5 --- |
...
input bool   TrendFollow          = false; // false = countertrend/fade (original 005 direction). true =
                                            // enter WITH the signal candle's direction instead (continuation
                                            // after the same extension+pullback), for trend-favoring symbols.
...
{
   Print("LIVE005 TREND M2 | ",event," | ",detail);
...
bool OpenCountertrend(Setup &s,MqlTick &tick)
{
   ...
   int dir=TrendFollow ? s.sigdir : -s.sigdir; // TrendFollow: bull signal -> BUY (continuation).
                                                // Countertrend (default): bull signal -> SELL (fade).
   if(dir==-1 && !AllowShort)
   {
...
int OnInit()
{
   ...
   f_log=FileOpen("XAU_M2_LIVE_005_TREND_LOG.csv",FILE_READ|FILE_WRITE|FILE_CSV|FILE_ANSI|FILE_SHARE_READ,',');
   ...
   Log("START",string("TF=M2 | PB=0.10R | SL=2R | own-Magic MAX1 | Magic=")+
       ...
       " | TrendFollow="+(TrendFollow?"true":"false")+
       " | AllowShort="+(AllowShort?"true":"false")+" | MinR_Points="+DoubleToString(MinR_Points,1)+
       " | orders="+(EnableLiveOrders?"ENABLED":"DRY"));
   Log("NOTE","TREND build: 005 setup detection + MinR_Points filter, with TrendFollow to enter WITH the signal direction (continuation) instead of fading it; live 0.01 TP1 uses whole-position +0.25R lock");
