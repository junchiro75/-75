//+------------------------------------------------------------------+
//| NAS_M2_BB_LIVE_005_RFILTER_V2.mq5                                |
//| NASDAQ (NAS100+) copy of XAU_M2_BB_LIVE_005_RFILTER_V2.mq5 --     |
//| identical logic (the code is symbol-agnostic via _Symbol), just   |
//| renamed to avoid confusion, with defaults set to the values       |
//| validated by ground-truth backtest specifically on NAS100+:       |
//| PullbackR=0.20, InitialSL_R=3.0, MinR_Points=6000,                 |
//| LatestSignalOnly=true (PF=1.458). NAS100's price scale/point      |
//| value is very different from gold's, so MinR_Points does NOT       |
//| transfer between symbols -- 6000 here is NASDAQ's own calibrated  |
//| value, not gold's.                                                 |
//| NOTE: AllowShort=false below still reflects GOLD's own finding     |
//| (SELL net -$540.86 vs BUY net +$1,697.03) -- this has not been     |
//| separately re-verified with a bucket breakdown on NAS100+ the way  |
//| gold's own SELL-fade bias was confirmed. Re-check before assuming  |
//| it transfers, the same way 001's AllowSellFade did NOT transfer    |
//| cleanly to NAS100.                                                  |
//| ---- inherited header from XAU_M2_BB_LIVE_005_RFILTER_V2.mq5 ----  |
...
input double PullbackR            = 0.20;
input double InitialSL_R          = 3.00;
input double TP1_R                = 0.50;
input double Lock_R               = 0.25;
input double TP2_R                = 0.90;
input double MinR_Points          = 6000; // NASDAQ-calibrated -- do NOT reuse gold's MinR_Points values,
                                           // NAS100's price scale/point value is very different.
input bool   LatestSignalOnly     = true; // true = a new BB-breakout signal candle discards any earlier
...
input ulong  MagicNumber          = 95014401; // distinct from all gold 005 variants so both symbols can
                                               // run on the same account without collision
input int    MaxDeviationPts      = 50;
input bool   EnableLiveOrders     = false; // SAFETY: set true only after checks
input bool   AllowShort           = false; // Carried over from GOLD's own finding (SELL net -$540.86 vs
                                            // BUY net +$1,697.03) -- NOT yet separately re-verified on
                                            // NAS100+. Re-check with a bucket breakdown before trusting this.
...
{
   Print("LIVE005 NAS RFILTER2 M2 | ",event," | ",detail);
...
bool OpenCountertrend(Setup &s,MqlTick &tick)
{
   ...
   bool ok=(dir==+1 ? trade.Buy(Lots,_Symbol,0.0,sl,tp,"LIVE005_NAS_M2")
                    : trade.Sell(Lots,_Symbol,0.0,sl,tp,"LIVE005_NAS_M2"));
   if(!ok)
   {
...
int OnInit()
{
   ...
   f_log=FileOpen("NAS_M2_LIVE_005_RFILTER2_LOG.csv",FILE_READ|FILE_WRITE|FILE_CSV|FILE_ANSI|FILE_SHARE_READ,',');
   ...
   Log("START",string("TF=M2 | PB=")+DoubleToString(PullbackR,2)+"R | SL="+DoubleToString(InitialSL_R,2)+
       "R | own-Magic MAX1 | Magic="+IntegerToString((int)MagicNumber)+" | Lots="+DoubleToString(Lots,2)+
   ...
   Log("NOTE","NAS100+ RFILTER2 build (ported from gold 005_RFILTER_V2): MinR_Points filter + LatestSignalOnly (newest BB-breakout candle supersedes any pending earlier setup)");
