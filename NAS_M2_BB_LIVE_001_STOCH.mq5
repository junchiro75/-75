//+------------------------------------------------------------------+
//| NAS_M2_BB_LIVE_001_STOCH.mq5                                       |
//| NASDAQ (NAS100+) port of the gold 001 strategy (XAU_M2_BB_LIVE_    |
...
//| raw point threshold and NAS100's price scale/point value is very   |
//| different from gold's -- it needs its own fresh sweep from 0.      |
//| AllowSellFade defaults to true (unbiased) until NAS100's own       |
...
//| time (own magic number, distinct from all gold 001 variants so    |
//| both symbols can run on the same account without collision).       |
...
input int    StochK_Period      = 16;   // matches gold chart setting (default MT5 is 5); re-check on NAS100's own chart
input int    StochD_Period      = 3;
input int    StochSlowing       = 3;
input double StochOverbought    = 70.0;
input double StochOversold      = 30.0;
input double SL_R               = 1.0;  // UNTUNED -- gold converged on 4.0, but re-verify for NAS100
input double TP_R               = 0.5;  // UNTUNED -- gold converged on 0.45
input double MinR_Points        = 0;    // UNTUNED -- gold values (300-900) do NOT transfer; NAS100's price
                                         // scale/point value is different. Start at 0 (no filter) and sweep
                                         // from scratch based on NAS100's own candle-body distribution.
input ulong  MagicNumber        = 96016101; // distinct from all gold 001 variants (950161xx) so both
                                             // symbols can run on the same account without collision
...
input bool   AllowSellFade      = true;  // UNBIASED default -- gold's AllowSellFade=false came from its
                                          ...
                                          // it transfers to NAS100 without separately verifying by backtest
...
{
   Print("NAS_M2_BB_LIVE_001_STOCH | ",event," | ",detail);
...
int OnInit()
{
   ...
   f_log=FileOpen("NAS_M2_BB_LIVE_001_STOCH_LOG.csv",FILE_READ|FILE_WRITE|FILE_CSV|FILE_ANSI|FILE_SHARE_READ,',');
