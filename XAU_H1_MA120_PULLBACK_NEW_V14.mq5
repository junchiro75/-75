//+------------------------------------------------------------------+
//| XAU_H1_MA120_PULLBACK_NEW_V14.mq5                                |
...
//| V13: added AllowShort (default true = identical to V12's behavior). |
//| For NAS100+ testing: mean-reversion (005-family) confirmed shorts   |
//| hurt there just like on gold (sell-only PF 0.67 vs buy-only 0.77);  |
//| given NAS100's structural long-side bias, this lets the SAME        |
//| trend-following logic be tested buy-only on that symbol without     |
//| touching the gold-validated V12 file.                               |
//| V14: added LatestSignalOnly, the same fix that dramatically         |
//| improved 005/007 on gold and 005 on NAS100 (MaxDD -71% on gold,     |
//| flipped 007 from loss to profit) -- applied here PER TIMEFRAME:     |
//| when a new H1 signal arms setups, any still-pending (not yet        |
//| entered) earlier setup for the SAME timeframe slot is discarded     |
//| first, so each of M1/M2/M3/M5 only ever watches its own latest H1   |
//| signal instead of letting several stale ones queue up. Default      |
//| false reproduces V13 exactly.                                       |
...
input bool   AllowShort            = true; // V13: false = skip H1 bear signals entirely (buy-only), matching
                                            // the 005-family's AllowShort. Default true reproduces V12 exactly.
input bool   LatestSignalOnly      = false; // V14: true = a new H1 signal discards any still-pending earlier
                                             // setup for the SAME timeframe slot (per-tf, since M1/M2/M3/M5
                                             // run independently). Default false reproduces V13 exactly.
...
void CheckH1Signal()
{
   ...
   if(dir==0) return;
   if(dir==-1 && !AllowShort) return; // buy-only mode: skip bear signals entirely
   ...
   for(int k=0;k<N_TF;k++)
   {
      ...
      if(LatestSignalOnly)
      {
         for(int i=ArraySize(setups)-1;i>=0;i--)
         {
            if(setups[i].tf==k)
            {
               Log("SIGNAL_SUPERSEDES","dropping pending setup for tf="+EnumToString(ENTRY_TFS[k])+
                   " (signal_time="+TS(setups[i].signal_time)+") for newer H1 signal");
               RemoveSetup(i);
            }
         }
      }
...
int OnInit()
{
   ...
   Log("START","BUILD=v14_latestsignalonly | EntryTFs(enabled)="+(UseM1?"M1 ":"")+(UseM2?"M2 ":"")+(UseM3?"M3 ":"")+(UseM5?"M5":"")+
       ...
       " | MA_Short="+IntegerToString(MA_Short)+" | AllowShort="+(AllowShort?"true":"false")+
       " | LatestSignalOnly="+(LatestSignalOnly?"true":"false")+
       " | StopLoss_R="+DoubleToString(StopLoss_R,2)+
