//+------------------------------------------------------------------+
//| XAU_H1_MA120_PULLBACK_NEW_V3.mq5                                 |
...
//| reality.                                                            |
//|                                                                    |
//| V3 FIX: V2's results showed 39% of trades exiting within the same  |
//| second at a loss of almost exactly spread+commission -- MA_Short   |
//| (20) was already BEHIND price at entry, so "TP" was trivially met  |
//| immediately. Root cause: a BUY setup is armed by an H1 BEAR signal |
//| (a decline), and in a decline the faster MA_Short typically drops  |
//| below MA_Long before price pulls back far enough to touch MA_Long  |
//| -- so entering at MA_Long already puts you past the MA_Short "TP". |
//| Fix: no entry (MA_Long touch, BB touch, or reentry) fires unless   |
//| MA_Short is still on the correct side of MA_Long (above for BUY,   |
//| below for SELL) at that moment -- i.e. the TP target must actually |
//| be ahead of price, not already behind it. Checked continuously, so |
//| a setup that isn't valid yet keeps waiting (up to MaxSetupHours)   |
//| rather than being dropped.                                         |
//|                                                                    |
//| UNVALIDATED: still not proven profitable, just no longer           |
//| structurally self-defeating. Test AllowReentryAtBB=true and false  |
//| in MT5 Strategy Tester to see which performs better.               |
//+------------------------------------------------------------------+
#property strict
...
void CheckSetups(MqlTick &tick)
{
   ...
   for(int i=ArraySize(setups)-1;i>=0;i--)
   {
      ...
      // TP-reachability filter: MA_Short must sit on the correct side of MA_Long for the
      // TP target to actually be ahead of price, not already behind it. In a decline (which
      // is what arms a BUY setup via an H1 bear signal), MA_Short often drops below MA_Long
      // before price pulls back down far enough to touch MA_Long -- entering there would put
      // the TP already behind us. Require MA_Short > MA_Long for BUY, < for SELL, checked at
      // the moment of any entry attempt (both the MA_Long and BB legs).
      bool tpReachable=(dir==+1 ? cur_ma_short[tf]>cur_ma_long[tf] : cur_ma_short[tf]<cur_ma_long[tf]);

      bool touchedMA=tpReachable && (dir==+1 ? tick.bid<=cur_ma_long[tf] : tick.ask>=cur_ma_long[tf]);
      bool touchedBB=tpReachable && (dir==+1 ? (tick.bid<=cur_e_l20[tf] || tick.bid<=cur_e_l4[tf])
...
int OnInit()
{
   ...
   Log("START","BUILD=v3_tp_reachability_filter | EntryTFs=M1+M2+M3 (all independent) | MA_Long="+IntegerToString(MA_Long)+
       " | MA_Short="+IntegerToString(MA_Short)+" | AllowReentryAtBB="+(AllowReentryAtBB?"true":"false")+
       " | EmergencySL_Points="+IntegerToString(EmergencySL_Points)+
