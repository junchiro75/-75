//+------------------------------------------------------------------+
//| XAU_M2_BB_LIVE_005_HYBRID.mq5                                    |
//| HYBRID variant of 005, built on top of the BUYONLY logic.        |
//|                                                                    |
...
//|   extension/pullback wait. The idea: a bull breakout the EA used   |
//|   to fade (SELL) is instead ridden (BUY), since ground-truth data  |
//|   showed SELL performs far worse than BUY in this secular gold     |
//|   uptrend.                                                          |
//|                                                                    |
...
input ulong  MagicNumber          = 95012201; // distinct from original 005 (95012001) and BUYONLY 005
                                               // (95012101) so all three can run side by side
...
struct Setup {
   ...
   int sigdir;              // always -1 (bear) now -- bull signals are handled immediately, never queued
...
{
   Print("LIVE005 HYBRID M2 | ",event," | ",detail);
...
void AddSignal(datetime sig,datetime close_time,int dir,double R,double c)
{
   ...
   Log("SIGNAL","BEAR R="+DoubleToString(R,_Digits)+
       " ext="+DoubleToString(setups[n].target,_Digits)+" -> wait for extension+pullback, then BUY");
}

// Shared order/virtual-position opener. dir is always +1 in this build (both the
// immediate bull-breakout entry and the bear-signal countertrend entry resolve to BUY).
bool OpenPosition(int dir,double R,MqlTick &tick)
{
   if(EnableLiveOrders)
   {
   ...
   if(!EnableLiveOrders)
   {
      v_open=true; v_tp1=false; v_dir=dir; v_R=R;
   ...
   double sl=ref-dir*InitialSL_R*R;
   double tp=ref+dir*TP2_R*R;
   ...
   bool ok=(dir==+1 ? trade.Buy(Lots,_Symbol,0.0,sl,tp,"LIVE005_M2_HYBRID")
                    : trade.Sell(Lots,_Symbol,0.0,sl,tp,"LIVE005_M2_HYBRID"));
   if(!ok)
   {
...
{
   int dir=-s.sigdir; // bear signal -> BUY (bull signals never reach this pipeline in this build)
   return OpenPosition(dir,s.R,tick);
}

void CheckNewM2Bar(MqlTick &tick)
{
   ...
   if(dir==+1)
   {
      ...
      Log("SIGNAL","BULL R="+DoubleToString(R,_Digits)+" -> immediate trend-follow BUY");
      OpenPosition(+1,R,tick);
      return;
   }

   AddSignal(sig,t,dir,R,c); // bear signal: unchanged extension+pullback wait -> countertrend BUY
}
...
int OnInit()
{
   ...
   f_log=FileOpen("XAU_M2_LIVE_005_HYBRID_LOG.csv",FILE_READ|FILE_WRITE|FILE_CSV|FILE_ANSI|FILE_SHARE_READ,',');
   ...
   Log("START",string("TF=M2 | HYBRID: bear->ext+pullback->BUY, bull->immediate trend BUY | own-Magic MAX1 | Magic=")+
       IntegerToString((int)MagicNumber)+" | Lots="+DoubleToString(Lots,2)+
       " | orders="+(EnableLiveOrders?"ENABLED":"DRY"));
   Log("NOTE","UNVALIDATED build: bull->immediate-BUY idea only tested in Python simulation so far");
