//+------------------------------------------------------------------+
//| XAU_M2_BB_LIVE_005_PARTIAL_V1.mq5                                  |
//| M2 Dual-BB -> +0.95R extension -> 0.10R pullback -> countertrend |
//| LIVE FORWARD TEST EA | own-Magic MAX1                             |
//| Sweep variant of 005_FINAL: adds PartialClosePct.                 |
//| PartialClosePct=0   -> identical to 005_FINAL (whole SL->+0.25R). |
//| PartialClosePct=0.3 -> close 30% at TP1, runner SL->+0.25R.       |
//| PartialClosePct=0.5 -> close 50% at TP1, runner SL->+0.25R.       |
//| NOTE: needs Lots big enough that Lots*PartialClosePct clears the  |
//| symbol's volume step (e.g. Lots=0.10 for 30/50 splits); otherwise |
//| the split is skipped and it silently falls back to lock-only.     |
//+------------------------------------------------------------------+
#property strict
...
input double PartialClosePct      = 0.0; // 0=off (FINAL-identical) | 0.3 | 0.5
input int    MaxExtensionHours    = 72;
input int    MaxPullbackHours     = 72;
input int    MaxVirtualExitHours  = 168;
input ulong  MagicNumber          = 95012301; // distinct from FINAL(95012001)/BUYONLY(95012101)/HYBRID(95012201)
...
{
   Print("LIVE005 PARTIAL M2 | ",event," | ",detail);
...
void ManagePosition(MqlTick &tick)
{
   ...
   {
      tp1_reached=true;
      Log("DRY_TP1",(PartialClosePct>0?"would close "+DoubleToString(PartialClosePct*100,0)+"% and ":"")+
          "move remaining SL to +0.25R");
      return;
   }
   ...
   if(PartialClosePct>0)
   {
      ...
      double closevol=MathFloor((vol*PartialClosePct)/step+1e-8)*step;
      closevol=NormalizeDouble(closevol,2);
      double remain=NormalizeDouble(vol-closevol,2);

      if(closevol>=minlot && remain>=minlot)
      {
         if(trade.PositionClosePartial(managed_ticket,closevol))
            Log("TP1_PARTIAL","closed "+DoubleToString(closevol,2)+" of "+DoubleToString(vol,2)+
                " ("+DoubleToString(PartialClosePct*100,0)+"%) | runner SL -> "+DoubleToString(managed_lock,_Digits));
         else
            Log("TP1_PARTIAL_FAIL",IntegerToString((int)trade.ResultRetcode())+" | "+trade.ResultRetcodeDescription()+
                " | falling back to whole-position lock");
      }
      else
         Log("TP1_PARTIAL_SKIP","Lots="+DoubleToString(vol,2)+" x "+DoubleToString(PartialClosePct,2)+
             " does not clear volume step="+DoubleToString(step,2)+" | falling back to whole-position lock");
   }

   if(SafeModifyPosition(managed_ticket,managed_dir,managed_lock,managed_tp2,"LOCK_MODIFY"))
   {
      tp1_reached=true;
      Log("TP1_LOCK","TP1 reached; remaining position SL moved to "+DoubleToString(managed_lock,_Digits));
   }
...
int OnInit()
{
   ...
   f_log=FileOpen("XAU_M2_LIVE_005_PARTIAL_LOG.csv",FILE_READ|FILE_WRITE|FILE_CSV|FILE_ANSI|FILE_SHARE_READ,',');
   ...
   Log("START",string("TF=M2 | PB=0.10R | SL=2R | own-Magic MAX1 | Magic=")+
       ...
       " | PartialClosePct="+DoubleToString(PartialClosePct,2)+
       " | orders="+(EnableLiveOrders?"ENABLED":"DRY"));
   Log("NOTE","PARTIAL sweep build of 005_FINAL; PartialClosePct=0 reproduces FINAL exactly (whole SL->+0.25R at TP1); >0 closes that fraction at TP1 then locks the runner");
